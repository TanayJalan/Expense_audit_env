...
def get_action(client, obs_dict):
    try:
...
{{
  ...
  "flagged_items": [
    {{"item_id": "<id>", "violation_type": "<type>", "reason": "<reason>", "confidence": 0.9}}
  ]
}}
...
def run_task(task_name, seed=42):
    ...
    try:
        ...
        action_dict = get_action(client, obs_dict)
        ...
        log_step(step=1, action=str(action_dict.get("decision")), reward=reward, done=done)
